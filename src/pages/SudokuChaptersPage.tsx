...
import { getSudokuProgress, type DifficultyProgress } from '../storage/db'
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <Link to="/sudoku" className="mt-1 inline-block text-[13px] font-semibold text-accent">
          Free play instead →
        </Link>
      </div>
      ...
      <div className="flex flex-col gap-6">
        ...
        {progress.hard && <EndlessCard hardCurrentLevelIndex={progress.hard.currentLevelIndex} gameRoute="/sudoku" />}
      </div>
...
function ChapterRow({ node }: { node: ChapterNodeState }) {
  ...
  if (status === 'current') {
    return (
      <Link to={`/sudoku/${difficulty}`} className={`${rowClass} transition hover:shadow-md`}>
        {content}
      </Link>
    )
  }
