...
import type { Difficulty } from '../engine/patches/types'
import { getPatchesProgress, type DifficultyProgress } from '../storage/db'
...
export default function PatchesChaptersPage() {
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <Link to="/patches" className="mt-1 inline-block text-[13px] font-semibold text-accent">
          Free play instead →
        </Link>
      </div>
      ...
      <div className="flex flex-col gap-6">
        ...
        {progress.hard && <EndlessCard hardCurrentLevelIndex={progress.hard.currentLevelIndex} gameRoute="/patches" />}
      </div>
...
function ChapterRow({ node }: { node: ChapterNodeState }) {
  ...
  if (status === 'current') {
    return (
      <Link to={`/patches/${difficulty}`} className={`${rowClass} transition hover:shadow-md`}>
        {content}
      </Link>
    )
  }
