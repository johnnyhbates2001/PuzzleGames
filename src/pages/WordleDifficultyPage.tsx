...
import type { Difficulty } from '../engine/wordle/types'
import { WORDLE_RULES } from '../engine/wordle/types'
import { averageTimeMs, getWordleProgress, type DifficultyProgress } from '../storage/db'
...
export default function WordleDifficultyPage() {
  const [progress, setProgress] = useState<Partial<Record<Difficulty, DifficultyProgress>>>({})
  const [rulesOpen, setRulesOpen] = useState(false)
  useAutoOpenRulesOnce('wordle', setRulesOpen)

  useEffect(() => {
    let cancelled = false
    Promise.all(DIFFICULTIES.map((d) => getWordleProgress(d))).then((results) => {
      if (cancelled) return
  ...
  return (
    ...
    >
      ...
      <h1 className="font-display text-[28px] font-extrabold">Wordle</h1>
      <div className="flex flex-col gap-3">
        {DIFFICULTIES.map((d) => {
          const p = progress[d]
          const avg = p ? averageTimeMs(p) : null
          const rules = WORDLE_RULES[d]
          return (
            <Link
              key={d}
              to={`/wordle/${d}`}
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="mt-0.5 text-[13px] text-ink-muted">
                  {rules.attempts} guesses{rules.hardMode ? ' · Hard mode' : ''}
                </p>
              </div>
