...
import type { Difficulty } from '../engine/zip/types'
import { getZipProgress, type DifficultyProgress } from '../storage/db'
...
export default function ZipChaptersPage() {
  const [progress, setProgress] = useState<Partial<Record<Difficulty, DifficultyProgress>>>({})
  const [rulesOpen, setRulesOpen] = useState(false)
  useAutoOpenRulesOnce('zip', setRulesOpen)

  useEffect(() => {
    let cancelled = false
    Promise.all(DIFFICULTIES.map((d) => getZipProgress(d))).then((results) => {
      if (cancelled) return
  ...
  return (
    ...
    >
      ...
      <div>
        <h1 className="font-display text-[28px] font-extrabold">Zip</h1>
        <Link to="/zip" className="mt-1 inline-block text-[13px] font-semibold text-accent">
          Free play instead →
        </Link>
      </div>
      ...
      <div className="flex flex-col gap-6">
        ...
        {progress.hard && <EndlessCard hardCurrentLevelIndex={progress.hard.currentLevelIndex} gameRoute="/zip" />}
      </div>
...
function ChapterRow({ node }: { node: ChapterNodeState }) {
  ...
  if (status === 'current') {
    return (
      <Link to={`/zip/${difficulty}`} className={`${rowClass} transition hover:shadow-md`}>
        {content}
      </Link>
    )
  }
