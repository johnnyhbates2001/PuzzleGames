...
export default function PatchesCompletePage() {
  ...
  const { timeMs, levelNumber, level, placed } = completion
    ...
    <main data-game="patches" className="fixed inset-0 overflow-hidden bg-bg">
      ...
      <div className="absolute inset-x-4 bottom-0 mx-auto flex max-w-lg flex-col items-center gap-4 rounded-t-[32px] bg-surface p-6 pb-[max(1.5rem,env(safe-area-inset-bottom))] text-center shadow-card">
        ...
        <div className="flex w-full flex-col gap-2">
          <button
            type="button"
            onClick={() => navigate(`/patches/${validDifficulty}`, { replace: true })}
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => navigate(`/patches/${validDifficulty}`, { state: { replayLevel: level }, replace: true })}
