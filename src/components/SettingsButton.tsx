  ...
  return (
    <>
      ...
      {shouldRender && (
        ...
        >
          <div
            className={`flex max-h-[min(85vh,42rem)] w-full max-w-sm flex-col rounded-t-3xl bg-surface shadow-card sm:rounded-3xl ${exiting ? 'anim-sheet-down' : 'anim-sheet-up'}`}
            style={dragY > 0 ? { transform: `translateY(${dragY}px)`, transition: dragging ? 'none' : 'transform 200ms ease-out' } : undefined}
            onClick={(event) => event.stopPropagation()}
          >
            <div className="shrink-0 rounded-t-3xl bg-surface px-5 pt-5 sm:rounded-3xl">
