    ...
    >
      {inPath && (
        <span className="pointer-events-none absolute inset-0">
          {connUp && <span className="absolute top-0 left-1/2 h-1/2 w-[28%] -translate-x-1/2 bg-accent" />}
          {connDown && <span className="absolute bottom-0 left-1/2 h-1/2 w-[28%] -translate-x-1/2 bg-accent" />}
          {connLeft && <span className="absolute top-1/2 left-0 h-[28%] w-1/2 -translate-y-1/2 bg-accent" />}
          {connRight && <span className="absolute top-1/2 right-0 h-[28%] w-1/2 -translate-y-1/2 bg-accent" />}
        </span>
      )}
      {inPath && checkpointNumber === null && (
        <span className={`relative z-10 rounded-full bg-accent ${isPathEnd ? 'size-[40%]' : 'size-[26%]'}`} />
      )}
