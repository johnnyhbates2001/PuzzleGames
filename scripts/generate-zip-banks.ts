...
const WALL_CLOCK_BUDGET_MS: Record<Difficulty, number> = { easy: 5 * 60 * 1000, medium: 10 * 60 * 1000, hard: 20 * 60 * 1000 }
...
function generateBank(difficulty: Difficulty, seed: number): ZipLevelRecord[] {
  ...
  while (levels.length < target) {
    ...
    if (levels.length % PROGRESS_EVERY === 0) {
      ...
      console.log(
        `[gen:zip-banks] ${difficulty}: ${levels.length}/${target} (${elapsedS}s elapsed, ${attempts} attempts)`,
      )
    }
  }

  const elapsedS = ((Date.now() - start) / 1000).toFixed(1)
  console.log(`[gen:zip-banks] ${difficulty}: done — ${levels.length}/${target} levels in ${elapsedS}s (${attempts} attempts)`)
  return levels
}
