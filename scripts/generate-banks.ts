...
function generateBank(difficulty: Difficulty, seed: number): LevelRecord[] {
  ...
  while (levels.length < TARGET_PER_DIFFICULTY) {
    ...
    if (levels.length % PROGRESS_EVERY === 0) {
      ...
      console.log(
        `[gen:banks] ${difficulty}: ${levels.length}/${TARGET_PER_DIFFICULTY} (${elapsedS}s elapsed, ${attempts} attempts)`,
      )
    }
  }

  const elapsedS = ((Date.now() - start) / 1000).toFixed(1)
  console.log(`[gen:banks] ${difficulty}: done — ${levels.length}/${TARGET_PER_DIFFICULTY} levels in ${elapsedS}s (${attempts} attempts)`)
  return levels
}
