...
function generateBank(difficulty: Difficulty, seed: number): NonogramLevelRecord[] {
  ...
  while (levels.length < TARGET_PER_DIFFICULTY) {
    ...
    if (levels.length % PROGRESS_EVERY === 0) {
      ...
      console.log(
        `[gen:nonogram-banks] ${difficulty}: ${levels.length}/${TARGET_PER_DIFFICULTY} (${elapsedS}s elapsed, ${attempts} attempts)`,
      )
    }
  }

  const elapsedS = ((Date.now() - start) / 1000).toFixed(1)
  console.log(`[gen:nonogram-banks] ${difficulty}: done — ${levels.length}/${TARGET_PER_DIFFICULTY} levels in ${elapsedS}s (${attempts} attempts)`)
  return levels
}
